...
// CO5300 initialization sequence matching working Arduino SH8601 
static const co5300_lcd_init_cmd_t amoled_init_cmds[] = 
{
  {0x11, (uint8_t []){0x00}, 0, 80},   // SLPOUT
  {0xC4, (uint8_t []){0x80}, 1, 0},    // CO5300-specific command
  {0x35, (uint8_t []){0x00}, 1, 0},    // TE (Tearing Effect) on
  {0x53, (uint8_t []){0x20}, 1, 1},    // WRCTRLD
  {0x63, (uint8_t []){0xFF}, 1, 1},    // CO5300-specific brightness command
  {0x51, (uint8_t []){0x00}, 1, 1},    // WRDISBV - set brightness low first
  {0x29, (uint8_t []){0x00}, 0, 10},   // DISPON
  {0x51, (uint8_t []){0xFF}, 1, 0},    // WRDISBV - set brightness high
};
...
void test_display_basic()
{
  ...
  const esp_lcd_panel_dev_config_t panel_config = {
    ...
    .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_BGR,  // Changed to BGR for correct colors
